...
public class EhdokkaatResti {
	...
	@Path("/poista/{param}")
	@Produces(MediaType.APPLICATION_JSON)
	public void poistaYksi(@PathParam("param") String ehdokasid) {
		EntityManager em = EmClass.getEm();
		System.out.println("-- delete Ehdokas by Id  --");
		em.getTransaction().begin();
		Query query = em.createQuery("DELETE FROM Ehdokkaat WHERE ehdokasId = :ehdokasId");
		query.setParameter("ehdokasId", 22); //Jos tätä numeroa muuttaa niin saa poistettua ehdokkaan
		query.executeUpdate();
		System.out.println("Toka");
		System.out.println("ehdokas deleted: " + ehdokasid);
