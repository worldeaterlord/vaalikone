...
<body>
	<div id="tuloksetWrapper">
		...
		<%
			...
			List<Vastaukset> parhaanEhdokkaanVastaukset = (List<Vastaukset>) request
					.getAttribute("parhaanEhdokkaanVastaukset");
		...
		%>
		<a href="Vaalikone?func=haeEhdokas&numero=<%=jarjestysnumero - 1%>">Edellinen
			ehdokas</a>&nbsp;
		<%
			}
			if (jarjestysnumero < 18) {
		%>
		<a href="Vaalikone?func=haeEhdokas&numero=<%=jarjestysnumero + 1%>">Seuraavaksi
			paras ehdokas</a>
		<%
			}
		...
		<h2>
			Numero:
			<%=seParasEhdokas.getEhdokasId()%></h2>
		<h3>
			Sinulle
			<%=jarjestysnumero + 1%>. paras ehdokas
		</h3>
		<h3>
			Yhteensopivuus:
			<%=prosentit%>%
